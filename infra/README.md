...
| <a name="input_log_analytics_workspace_name"></a> [log\_analytics\_workspace\_name](#input\_log\_analytics\_workspace\_name) | Name of the existing Log Analytics workspace, in resource\_group\_name, that Key Vault diagnostic logs (all categories) and metrics are sent to. Leave empty to skip diagnostic settings. | `string` | `""` | no |

## Outputs

| Name | Description |
|------|-------------|
| <a name="output_APICENTER_DATA_API_HOSTNAME"></a> [APICENTER\_DATA\_API\_HOSTNAME](#output\_APICENTER\_DATA\_API\_HOSTNAME) | n/a |
